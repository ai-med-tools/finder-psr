...
export class M2 {
  ...
  public static calcAccuracyOfLocalizationOfSymptoms(
    expertMarkup: any[],
    memberMarkup: any[],
  ) {
    ...
    const algStarter = new HungarianAlgWithloss(
      expertMarkup,
      memberMarkup,
      true,
    );
    ...
    const preparedExpertsTokenized =
      Tokenizer.prepareToTokenize(arExpertsFragments);
    const resultExpertsTokenized = Tokenizer.tokenize(
      preparedExpertsTokenized,
    ).flat();
    const preparedMemberTokenized =
      Tokenizer.prepareToTokenize(arMemberFragments);
    ...
    const { aggregationOfAccuracyCompleteness } =
      M1.comparingTwoTokinzedMarkups(arMemberFragments, arExpertsFragments);
    return aggregationOfAccuracyCompleteness / delimeter;
  }
}
